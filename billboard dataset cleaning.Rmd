...
genre_summary = read_csv(file = "./data/word_bank_genre_code_ling.csv",
                         col_names = TRUE,
                         na = " ") %>%
  select(-...1) %>%
  group_by(genre_cate) %>%
  summarise(subgenre = list(value))

write_csv(genre_summary, file = "./data/word_bank_genre_summary_ling.csv")
